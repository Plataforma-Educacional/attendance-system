    ...
    <Link to={routes.chat({ id: chat.id })}>
      <div className="group flex items-center justify-center space-x-10 border-l-2 border-white py-4 pl-2.5 pr-1.5 hover:bg-[#5766DA] ">
        ...
        <div className=" h-full flex-1">
          {chat.status === 'active' && (
            <div className="rounded-full bg-[#A3ECB8] px-5 py-2.5 text-center text-lg font-bold text-[#053F04]">
              Aberto
            </div>
          )}
          {chat.status === 'close' && (
            <div className="rounded-full bg-[#D62828] px-5 py-2.5 text-center text-lg font-bold text-[#FFD3D3]">
              Fechado
            </div>
          )}
          {chat.status === 'waiting' && (
            <div className="rounded-full bg-[#ABC3FF] px-5 py-2.5 text-center text-lg font-bold text-[#313978]">
              Pendente
            </div>
          )}
        </div>
